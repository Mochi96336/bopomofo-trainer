...
const ANALYSIS_V2_PREFERENCES_KEY = "bopomofo-trainer.analysis-v2.v1";
...
const BODY_SIZE_BUCKETS: readonly CoordinationBodySizeBucket[] = ["2", "3", "4+"];
...
function loadPreferences(storage: AnalysisV2PreferenceStorage): AnalysisV2Preferences {
  ...
  try {
    source = storage.getItem(ANALYSIS_V2_PREFERENCES_KEY);
  } catch {
...
function savePreferences(
  ...
  try {
    storage.setItem(ANALYSIS_V2_PREFERENCES_KEY, JSON.stringify(preferences));
  } catch {
    // The view remains usable in a storage-degraded session.
  }
}
...
function tabLabel(tab: AnalysisV2Tab): string {
  if (tab === "coordination") return "協調";
  if (tab === "strategy") return "策略";
  return "語意";
}
...
function keyDetailMarkup(model: AnalysisV2Model, tokenId: TokenId | null): string {
  if (tokenId === null) {
    return `<div class="analysis-v2-selection-empty">選一個按鍵查看累積資料與最近趨勢。</div>`;
  }
  ...
  const correctness = progress?.correctness.points.slice(-5).map((point) =>
    `${Math.round(point.value * 100)}%`,
  ) ?? [];
  const timing = progress?.timing.points.slice(-5).map((point) =>
    `${Math.round(point.value)} ms`,
  ) ?? [];
  return `<article class="analysis-v2-detail-card">
    <div class="analysis-v2-detail-heading">
      <strong>${escapeHtml(row.symbol)}</strong>
      <span>實體鍵 ${escapeHtml(row.physicalKey)}</span>
    </div>
    <dl>
...
function correctnessKeyboardMarkup(
  ...
  return `<div class="analysis-v2-semantic-layout">
    <section class="analysis-v2-keyboard-wrap" aria-label="按鍵錯誤觀察比例">
      <div class="analysis-v2-keyboard">
        ${KEYBOARD_GEOMETRY_ROWS.map((row) => `<div class="analysis-v2-keyboard-row">
          ${row.map((key) => {
...
interface ConfusionToken {
  readonly tokenId: TokenId;
  readonly label: string;
  readonly count: number;
}

function confusionTokens(model: AnalysisV2Model): readonly ConfusionToken[] {
  const counts = new Map<TokenId, number>();
  for (const row of model.semantic.confusions) {
    counts.set(row.expectedTokenId, (counts.get(row.expectedTokenId) ?? 0) + row.occurrences);
    counts.set(row.actualTokenId, (counts.get(row.actualTokenId) ?? 0) + row.occurrences);
  }
...
function confusionMatrixMarkup(model: AnalysisV2Model): string {
  const tokens = confusionTokens(model);
  if (tokens.length === 0) {
    return `<div class="analysis-v2-empty">尚無可歸因的誤按資料。多個候選仍可能成立時，系統不會猜測你原本想按哪一鍵。</div>`;
  }
  ...
  return `<div class="analysis-v2-matrix-scroll">
    <table class="analysis-v2-matrix confusion-matrix">
      ...
      <tbody>${tokens.map((expected) => `<tr>
        <th scope="row">${escapeHtml(expected.label)}</th>
        ${tokens.map((actual) => {
          ...
          return `<td style="--analysis-strength:${strength}" title="${escapeHtml(expected.label)} → ${escapeHtml(actual.label)}：${count} 次">${count === 0 ? "·" : count}</td>`;
        }).join("")}
      </tr>`).join("")}</tbody>
    </table>
  </div>`;
...
function semanticMarkup(
  ...
  return `<section class="analysis-v2-domain" aria-labelledby="analysis-v2-semantic-title">
    <div class="analysis-v2-domain-head">
      <div><h3 id="analysis-v2-semantic-title">語意輸入</h3><p>只回答「哪個符號容易按錯、會誤成什麼」，不把結構相鄰當成實際動作。</p></div>
      <div class="analysis-v2-segments" role="group" aria-label="語意分析視圖">
...
function motorCellMarkup<Scope>(cell: AnalysisV2MotorCell<Scope> | undefined): string {
  ...
  const state = cell.ready ? "ready" : "sampling";
  const value = cell.currentTimeToTypeMs === null ? "—" : milliseconds(cell.currentTimeToTypeMs);
  return `<div class="analysis-v2-motor-cell ${state}">
    <strong>${escapeHtml(value)}</strong>
    <small>${cell.timingSamples} 個乾淨樣本 · ${cell.observations} 次觀察</small>
    <span>${escapeHtml(trendText(cell))}</span>
  </div>`;
}
...
function findImmediate(
  ...
  return model.coordination.immediateHands.find((cell) =>
    cell.scope.fromHand === fromHand && cell.scope.toHand === toHand,
  );
}
...
function immediateHandMatrix(model: AnalysisV2Model): string {
  ...
  return `<section class="analysis-v2-card">
    <h4>標準指法手別轉換</h4>
    <p>依實體鍵的標準指法分工推定，不是偵測實際使用的手。</p>
    <table class="analysis-v2-motor-table">
      <thead><tr><th>前一鍵 ↓ / 下一鍵 →</th>${hands.map((hand) => `<th>${label(hand)}</th>`).join("")}</tr></thead>
      <tbody>${hands.map((from) => `<tr><th>${label(from)}</th>${hands.map((to) => `<td>${motorCellMarkup(findImmediate(model, from, to))}</td>`).join("")}</tr>`).join("")}</tbody>
    </table>
  </section>`;
}
...
function findCoordination(
  ...
  return model.coordination.coordination.find((cell) =>
    cell.scope.bodySize === bodySize && cell.scope.handShape === handShape,
  );
}

function coordinationFacet(model: AnalysisV2Model): string {
  const shapes: readonly CoordinationAggregateScope["handShape"][] = [
    "left-only", "right-only", "mixed", "unknown",
  ];
  ...
  return `<section class="analysis-v2-card">
    <h4>音節內協調</h4>
    <p>從第一個已接受的 body 成分到最後一個 body 成分；不同成分數分開看。</p>
    <table class="analysis-v2-motor-table wide">
      <thead><tr><th>body 成分</th>${shapes.map((shape) => `<th>${shapeLabel[shape]}</th>`).join("")}</tr></thead>
      <tbody>${BODY_SIZE_BUCKETS.map((size) => `<tr><th>${size}</th>${shapes.map((shape) => `<td>${motorCellMarkup(findCoordination(model, size, shape))}</td>`).join("")}</tr>`).join("")}</tbody>
    </table>
  </section>`;
}
...
function findRevisit(
  model: AnalysisV2Model,
  hand: "left" | "right",
  oppositeHandIntervened: boolean,
): AnalysisV2MotorCell<SameHandRevisitAggregateScope> | undefined {
  return model.coordination.sameHandRevisits.find((cell) =>
    cell.scope.hand === hand && cell.scope.oppositeHandIntervened === oppositeHandIntervened,
  );
}
...
function revisitMatrix(model: AnalysisV2Model): string {
  ...
  return `<section class="analysis-v2-card">
    <h4>同側鍵位再出手</h4>
    <p>比較同一標準指法側再次出現時，中間是否曾出現另一側鍵位。</p>
    <table class="analysis-v2-motor-table">
      <thead><tr><th>鍵位側</th><th>連續同側</th><th>中間有另一側</th></tr></thead>
      <tbody>${hands.map((hand) => `<tr><th>${hand === "left" ? "左側" : "右側"}</th><td>${motorCellMarkup(findRevisit(model, hand, false))}</td><td>${motorCellMarkup(findRevisit(model, hand, true))}</td></tr>`).join("")}</tbody>
    </table>
  </section>`;
}

function toneCommitCards(model: AnalysisV2Model): string {
  const cells = [...model.coordination.toneCommits].sort((left, right) =>
    left.scope.toneToken.localeCompare(right.scope.toneToken),
  );
  return `<section class="analysis-v2-card">
    <h4>聲調完成</h4>
    <p>最後一個 body 成分到聲調鍵的乾淨時間，各聲調分開估計。</p>
    <div class="analysis-v2-tone-grid">
      ${cells.length === 0
        ? '<div class="analysis-v2-empty compact">尚無聲調完成資料。</div>'
        : cells.map((cell) => `<div class="analysis-v2-tone-cell"><b>${escapeHtml(tokenLabel(cell.scope.toneToken))}</b>${motorCellMarkup(cell)}</div>`).join("")}
    </div>
  </section>`;
}

function coordinationMarkup(model: AnalysisV2Model): string {
  return `<section class="analysis-v2-domain" aria-labelledby="analysis-v2-coordination-title">
    <div class="analysis-v2-domain-head"><div>
      <h3 id="analysis-v2-coordination-title">動作協調</h3>
      <p>${model.coordination.readyScopes} 類已有至少 5 個乾淨樣本 · ${model.coordination.cleanTimingSamples} 個乾淨時間樣本。不同動作類型不以絕對毫秒互相比弱。</p>
    </div></div>
    <div class="analysis-v2-card-grid">
      ${immediateHandMatrix(model)}
      ${revisitMatrix(model)}
      ${coordinationFacet(model)}
      ${toneCommitCards(model)}
    </div>
  </section>`;
}

function positionLabel(position: InputOrderPositionAggregateScope["canonicalPosition"]): string {
  if (position === "first") return "前";
  if (position === "last") return "後";
  return "中";
}
...
function positionCount(
  model: AnalysisV2Model,
  bodySize: CoordinationBodySizeBucket,
  canonicalPosition: InputOrderPositionAggregateScope["canonicalPosition"],
  acceptedPosition: InputOrderPositionAggregateScope["acceptedPosition"],
): number {
  return model.strategy.inputOrderPositions.find((aggregate) =>
    aggregate.scope.bodySize === bodySize
    && aggregate.scope.canonicalPosition === canonicalPosition
    && aggregate.scope.acceptedPosition === acceptedPosition,
  )?.observations ?? 0;
}
...
function strategyMatrix(model: AnalysisV2Model, bodySize: CoordinationBodySizeBucket): string {
  const rows = POSITIONS.map((canonical) => {
    ...
    return { canonical, values, total };
  });
  const matrixTotal = rows.reduce((sum, row) => sum + row.total, 0);
  return `<section class="analysis-v2-card strategy-card">
    <div class="analysis-v2-card-title-line"><h4>${bodySize} 成分 body</h4><span>${matrixTotal} 個位置觀察</span></div>
    <table class="analysis-v2-matrix strategy-matrix">
      <caption>列是 canonical 位置；欄是實際被接受的位置。</caption>
      <thead><tr><th>canonical ↓ / 實際 →</th>${POSITIONS.map((position) => `<th>${positionLabel(position)}</th>`).join("")}</tr></thead>
      <tbody>${rows.map((row) => `<tr><th>${positionLabel(row.canonical)}</th>${row.values.map((count) => {
...
function strategyMarkup(model: AnalysisV2Model): string {
  return `<section class="analysis-v2-domain" aria-labelledby="analysis-v2-strategy-title">
    <div class="analysis-v2-domain-head"><div>
      <h3 id="analysis-v2-strategy-title">輸入策略</h3>
      <p>${model.strategy.totalObservations} 個位置觀察。canonical 位置只是注音結構的參考座標，不代表要求的輸入順序；實際位置來自你真正完成 body 的順序。</p>
    </div></div>
    <div class="analysis-v2-strategy-grid">
      ${BODY_SIZE_BUCKETS.map((size) => strategyMatrix(model, size)).join("")}
    </div>
    <p class="analysis-v2-note">4+ 成分為了讓長期資料保持 bounded，中間位置會合併成同一格；不保存 raw trace，也不建立 token-pair 順序分數。</p>
  </section>`;
}

function analysisBodyMarkup(
  model: AnalysisV2Model,
  preferences: AnalysisV2Preferences,
  selectedKey: TokenId | null,
): string {
...
export function renderAnalysisV2Summary(
  ...
  section.innerHTML = `<div class="diagnostic-summary-heading">
      <div>
        <h3>學習分析</h3>
        <p>${model.semantic.keysWithData} 鍵有語意資料 · ${model.coordination.readyScopes} 類協調樣本已可比較 · ${model.strategy.totalObservations} 個順序位置觀察</p>
      </div>
      <button type="button" class="diagnostic-open-analysis">進入分析</button>
    </div>
    <div class="diagnostic-summary-signals" aria-label="學習分析摘要">
      <div><span>語意</span><strong>${model.semantic.keysWithData} 鍵</strong><small>${model.semantic.repeatedConfusions} 組重複誤按</small></div>
      <div><span>協調</span><strong>${model.coordination.readyScopes} 類</strong><small>${model.coordination.cleanTimingSamples} 個乾淨時間樣本</small></div>
      <div><span>策略</span><strong>${model.strategy.totalObservations}</strong><small>${model.strategy.bodySizeBucketsWithData} 種 body 尺度有資料</small></div>
    </div>`;
...
export function createAnalysisV2(options: AnalysisV2Options): AnalysisV2Controller {
  ...
  const persist = (): void => savePreferences(options.storage, preferences);

  const render = (): void => {
    host.innerHTML = `<div class="analysis-v2-shell">
      <header class="analysis-v2-header">
        <div><p class="analysis-v2-kicker">Analysis V2</p><h2 id="diagnostic-analysis-title">學習分析</h2></div>
        <div class="analysis-v2-header-actions">
          <div class="analysis-v2-tabs" role="tablist" aria-label="分析類型">
            ${TABS.map((tab) => `<button type="button" role="tab" data-action="select-tab" data-tab="${tab}" aria-selected="${preferences.activeTab === tab}" tabindex="${preferences.activeTab === tab ? 0 : -1}">${tabLabel(tab)}</button>`).join("")}
          </div>
          <button type="button" class="diagnostic-analysis-close" data-action="close-analysis" aria-label="返回練習">Esc</button>
        </div>
      </header>
      <main class="analysis-v2-main">${analysisBodyMarkup(model, preferences, selectedKey)}</main>
    </div>`;
  };
  ...
  const selectTab = (tab: AnalysisV2Tab, focus = true): void => {
    ...
    render();
    if (focus) {
      host.querySelector<HTMLButtonElement>(`[data-action="select-tab"][data-tab="${tab}"]`)
        ?.focus();
    }
  };
  ...
  host.onclick = (event) => {
    ...
    if (action === "semantic-view") {
      const value = target.dataset.value;
      if (!isSemanticView(value)) return;
