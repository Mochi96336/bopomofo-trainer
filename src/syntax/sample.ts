...
const samplerExcludedRuleClasses = new WeakMap<
  ProductionConstituent,
  ReadonlySet<ProductionRuleClass>
>();

function excludedRuleClassesForSampling(
  constituent: ProductionConstituent,
): ReadonlySet<ProductionRuleClass> {
  const cached = samplerExcludedRuleClasses.get(constituent);
  if (cached !== undefined) return cached;
  const prepared = excludedClassesForConstituent(constituent);
  samplerExcludedRuleClasses.set(constituent, prepared);
  return prepared;
}
  ...
  for (const constituent of ordered) {
    ...
    for (let occurrenceIndex = 0; occurrenceIndex < count; occurrenceIndex += 1) {
      ...
      const child = sampleCategory(
        ...
        excludedRuleClassesForSampling(constituent),
