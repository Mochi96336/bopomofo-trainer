...
for (const node of profile.nodes ?? []) {
  const frame = node.callFrame ?? {};
  const url = frame.url ?? "";
  if (!url.endsWith("/src/syntax/sample.ts")) continue;
...
lineTicks.sort((a, b) => b.ticks - a.ticks);
console.log(JSON.stringify({ topSampleLineTicks: lineTicks.slice(0, 80) }, null, 2));
