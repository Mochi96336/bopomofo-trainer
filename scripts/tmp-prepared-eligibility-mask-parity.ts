...
const manualContext: PreparedStructuralSamplingContext = {
...
const sentenceRuleIds = FORMAL_SYNTAX_RULES
  ...
  .map((rule) => rule.id);
for (const [ruleIndex, ruleId] of sentenceRuleIds.entries()) {
