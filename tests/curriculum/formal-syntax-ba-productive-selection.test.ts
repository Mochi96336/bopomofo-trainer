...
// Keep this rule bundle aligned with the canonical BA composition regression in
// #235. This PR is testing the selector seam, not whether a separately minimized
// grammar bundle happens to preserve the same executable closure.
...
describe("productive BA construction selection", () => {
  it("activates an unseen patient-taking predicate through explicit productive BA practice", () => {
    ...
    // Subject and DisposalPatient are syntax-only nominal slots. This proof must
    // not use semantic plausibility to force which noun fills which role.
