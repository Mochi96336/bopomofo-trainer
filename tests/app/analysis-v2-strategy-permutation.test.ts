...
describe("Analysis V2 Strategy interpretation", () => {
  it("describes three-part strategy as whole-word reordering across all six orders", () => {
    ...
    expect(host.querySelectorAll(".strategy-order-table tbody tr")).toHaveLength(6);
    expect(host.querySelector(".strategy-order-table")?.textContent).toContain("前 → 中 → 後");
    expect(host.querySelector(".strategy-order-table")?.textContent).toContain("中 → 前 → 後");
    expect(host.querySelector(".analysis-v2-strategy-readout")?.textContent)
      .toContain("中 → 前 → 後 9%");
    expect(host.querySelector(".analysis-v2-method")?.textContent)
      .toContain("其餘五種都算換序輸入");
  });

  it("summarizes two-part strategy by total position deviation instead of picking a 1-point winner", () => {
    controller = createAnalysisV2({ getModel: () => model(), storage: memoryStorage() });
    controller.open("strategy");
    const host = controller.host;
  ...
  it("never reconstructs complete historical order from old position marginals", () => {
