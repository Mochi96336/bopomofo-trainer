...
describe("pinned GSD AUX source inventory semantics", () => {
  ...
  it("records exact-aux position separately from lexical form identity", () => {
    const audit = summarizeModalitySourceEvidence([SOURCE]);

    expect(audit.auxRelationHeadPositionCounts).toEqual({
      postverbal: 1,
      preverbal: 2,
    });
    expect(audit.preverbalAuxTokenCount).toBe(2);
    expect(audit.postverbalAuxTokenCount).toBe(1);
    expect(audit.preverbalAuxFormCounts).toEqual({
      會: 1,
      應該: 1,
    });
    expect(audit.postverbalAuxFormCounts).toEqual({ 了: 1 });
    expect(audit.preverbalAuxFeatureCounts).toEqual({
      "Mood=Nec": 1,
      "VerbType=Mod": 1,
    });
    expect(audit.postverbalAuxFeatureCounts).toEqual({ "Aspect=Perf": 1 });
  });
