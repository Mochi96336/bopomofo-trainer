...
Two shortcuts named in `docs/weakness-diagnostics.md` are untouched and still real: the analysis rebuilds its model from mirrored `localStorage` rather than composing from live product state, and the relationship overlay reads inspector rows back out of the DOM.
