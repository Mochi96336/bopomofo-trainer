...
`F8` 預覽在開發與 production 都保留。開發模式另有 `F9` 暫時開放稀有度與 `F10` 自動走完目前句子的檢視工具；production build 會在這兩個事件抵達產品 listener 前隔離它們，因此線上網站不會以會改變學習資料的檢視操作寫入真實進度。隔離層不取消瀏覽器預設行為，因此被隔離的功能鍵仍可由瀏覽器或作業系統處理。
